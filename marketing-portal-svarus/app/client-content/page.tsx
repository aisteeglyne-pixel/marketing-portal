              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                ...
                {post.publish_date && (
                  <span style={{ fontSize: 12, color: '#888' }}>
                    Planuojama: {new Date(post.publish_date).toLocaleDateString('lt-LT')}
                  </span>
                )}
              </div>

              {post.caption && (
                <p style={{ fontSize: 14, color: '#555', marginBottom: '1rem', lineHeight: 1.6 }}>
                  {post.caption}
                </p>
              )}
              ...
              {post.status === 'review' && (
                <div style={{ borderTop: '1px solid #f0f0f0', paddingTop: '1rem' }}>
                  <textarea
                    ...
                    placeholder="Komentaras agentūrai (neprivaloma)..."
                    rows={2}
                    style={{
