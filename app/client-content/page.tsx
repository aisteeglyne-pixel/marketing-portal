              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                <div>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                    <span className={`badge badge-${post.status}`}>
                      {lt.clientContent.statuses[post.status as keyof typeof lt.clientContent.statuses] || post.status}
                    </span>
                ...
                {post.publish_date && (
                  <span style={{ fontSize: 12, color: '#888' }}>
                    {lt.clientContent.plannedDate} {new Date(post.publish_date).toLocaleDateString('lt-LT')}
                  </span>
                )}
              </div>
              {post.caption && (
                <p style={{ fontSize: 14, color: '#555', marginBottom: '1rem', lineHeight: 1.6 }}>{post.caption}</p>
              )}
              {post.status === 'review' && (
                <div style={{ borderTop: '1px solid #f0f0f0', paddingTop: '1rem' }}>
                  <textarea
                    ...
                    placeholder={lt.clientContent.commentPlaceholder}
                    rows={2}
                    style={{ width: '100%', padding: '8px 12px', border: '1px solid #e5e5e5', borderRadius: 8, fontSize: 13, resize: 'vertical', outline: 'none', marginBottom: '0.75rem' }}
                  />
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button
                      onClick={() => updateStatus(post.id, 'approved')}
                      disabled={loading === post.id}
                      style={{ background: '#EAF3DE', color: '#27500A', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
                      {lt.clientContent.approveButton}
                    </button>
                    <button
                      onClick={() => updateStatus(post.id, 'rejected')}
                      disabled={loading === post.id}
                      style={{ background: '#FCEBEB', color: '#791F1F', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
                      {lt.clientContent.rejectButton}
                    </button>
                  </div>
                </div>
              )}
