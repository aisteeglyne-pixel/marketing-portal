...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (authError) { setError('Neteisingas el. paštas arba slaptažodis'); setLoading(false); return }
    if (data.user) {
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f8f8f7' }}>
      <div className="card" style={{ width: '100%', maxWidth: 420 }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 600 }}>Klientų portalas</h1>
        ...
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div>
            ...
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} required style={{ width: '100%', padding: '10px 12px', border: '1px solid #e5e5e5', borderRadius: 8, fontSize: 14, outline: 'none' }} placeholder="••••••••" />
          </div>
